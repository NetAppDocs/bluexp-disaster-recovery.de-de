...
BlueXP Disaster Recovery ist ein Cloud-basierter Disaster Recovery Service, der Disaster Recovery Workflows automatisiert. Zunächst können Sie mit dem Disaster Recovery-Service von BlueXP Ihre lokalen, NFS-basierten Workloads schützen und in einer Technologievorschau VMware vSphere VMFS-Datastores (Virtual Machine File System) mit NetApp Storage in eine der folgenden Systeme einbinden:
